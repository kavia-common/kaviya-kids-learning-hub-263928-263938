...
export default function WorldMapPage() {
  ...
  // Backend dashboard fetch (prefer backend, fall back to local)
  useEffect(() => {
    ...
    dashboardService.getDashboard(userId)
      .then((data) => {
        ...
        const l = Number(data?.level ?? level);
        const b = Array.isArray(data?.badges) ? data.badges : badges;

        if (u) {
          const cached = { ...(profile || {}), username: u };
          setProfile(cached);
          try {
        ...
        setBadges(b);
        try {
          localStorage.setItem('xp', String(x));
          localStorage.setItem('level', String(l));
          localStorage.setItem('badges', JSON.stringify(b));
        } catch {}
      })
      .catch((e) => {
        setErr(e?.message || 'Could not sync latest progress. Showing last saved progress.');
      })
      ...
      <div style={styles.container}>
        <header style={styles.header}>
          <div style={styles.headerLeft}>
            ...
            <div>
              ...
              <p style={styles.subtitle}>
                {loading ? 'Syncing...' : `XP: ${xp} • Level: ${level}`}
              </p>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={styles.progressPill} title="XP unlocks more islands">
              <span style={styles.pillEmoji} aria-hidden="true">
                ✨
              </span>
        ...
        {showChallenges && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowChallenges(false);
            }}
          >
            <div
              style={{
                                ...
                                >
                                  ...
                                  {c.dueBy && (
                                    <span style={{ fontSize: 12, color: '#1D4ED8' }}>
                                      Due by {new Date(c.dueBy).toLocaleDateString()}
                                    </span>
                                  )}
                                </div>
                                {c.message && (
                                  <div style={{ color: '#374151', marginTop: 4, fontSize: 14 }}>
                                    “{c.message}”
                                  </div>
                                )}
                              </li>
        ...
        <section aria-label="Learning World Islands" style={styles.grid}>
          {islands.map((island) => {
            const isLocked = !island.unlocked;
            return (
              <article key={island.id} style={{ ...styles.card, background: '#fff' }} aria-labelledby={`${island.id}-title`}>
                <div style={{ ...styles.cardTop, background: island.gradient }} aria-hidden="true">
                  <div
                    style={{
                      ...styles.icon,
        ...
        <section aria-label="Map legend and tips" style={styles.tipsCard}>
          <div style={styles.tipsHeader}>
            ...
            <span style={styles.tipsEmoji} aria-hidden="true">
              🧭
            </span>
          </div>
        ...
        <section aria-label="Collections" style={styles.tipsCard}>
          ...
          <p style={{ margin: 0, color: '#374151', fontSize: 14 }}>Visit your Sticker Book to arrange your rewards!</p>
...
const styles = {
  ...
  title: { margin: 0, fontSize: 24, color: '#1E3A8A', letterSpacing: '-0.01em' },
  subtitle: { margin: '4px 0 0', color: '#374151', fontSize: 14 },
  progressPill: {
    border: '2px solid #F59E0B',
    background: '#FFFBEB',
    color: '#92400E',
  ...
  cardTop: { minHeight: 80, position: 'relative', display: 'grid', placeItems: 'center' },
  icon: {
    height: 68,
    width: 68,
    borderRadius: 18,
  ...
  cardBody: { display: 'grid', gap: 8, padding: 14, alignContent: 'start' },
  cardTitle: { margin: 0, fontSize: 18, color: '#111827' },
  cardDesc: { margin: 0, color: '#374151', fontSize: 14 },
  metaRow: { display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 4, flexWrap: 'wrap' },
  metaLabel: { fontWeight: 800, color: '#1E3A8A', fontSize: 13 },
  lockBadge: {
  ...
  tipsHeader: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 },
  tipsTitle: { margin: 0, fontSize: 18, color: '#111827' },
  tipsEmoji: { fontSize: 20 },
  tipsList: { margin: '8px 0 0', padding: '0 18px', color: '#374151', fontSize: 14, lineHeight: 1.6 }
};
